...
async def entrypoint(ctx: agents.JobContext):
    ...
    session = AgentSession(
        stt=google.STT(model="latest-short", languages="en-US", credentials_file=os.getenv("GOOGLE_APPLICATION_CREDENTIALS")),
        llm=google.beta.realtime.RealtimeModel(
            model="gemini-2.0-flash-live-001",
            voice="Puck",
            temperature=0.8,
            instructions="You are a helpful multimodal assistant.",
            modalities=["TEXT", "AUDIO", "IMAGE"],
            api_key=os.getenv("GOOGLE_API_KEY"),
            language="en-US",
        ),
        tts=google.TTS(language="en-US", voice_name="en-US-Wavenet-D", credentials_file=os.getenv("GOOGLE_APPLICATION_CREDENTIALS")),
        vad=silero.VAD.load(),
